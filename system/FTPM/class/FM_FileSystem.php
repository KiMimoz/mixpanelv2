...
/* PUBLIC METHODS ****************************************************************************** */
	...
	public function ftpConnect($host, $port = 21, $timeout = 30)
	{
		...
		if($this->_FileManager->ftpSSL) {
			...
			{
				$this->_Log->add('No FTPS support on this server', 'error');
			}
		}
		else
		{
		...
		this::show_toastr('error', 'Your ftp server is inaccessible!', '<b>Error</b>', 1);
		return false;
	}
	...
	public function ftpLogin($user, $password)
	{
		...
		this::show_toastr('error', 'Your ftp login data are incorrects!', '<b>Error</b>', 1);
