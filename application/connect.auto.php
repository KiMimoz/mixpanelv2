...
class connect extends Capsule
{
	...
	{
		self::$query_id = self::$g_con->prepare($query);//ehhh
	...
	public static function rows2($table,$arg)
	{
		if(is_array($table))
		{
			$rows = 0;
			...
			foreach($table as $val)
			{
				$q = self::$g_con->prepare("SELECT * FROM `".$val."` ".$arg."");
				$q->execute();
				$rows += $q->rowCount();
			}
		...
		$q = self::$g_con->prepare("SELECT * FROM `".$table."` ".$arg."");
		$q->execute();
